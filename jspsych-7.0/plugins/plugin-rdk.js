var jsPsychRdk = (function (jspsych) {
    ...
    const info = {
        ...
        parameters: {
            ...
            /** The proportion of dots that use the primary shape (dot_shape); remaining dots use the other shape. 1.0 = all dots use dot_shape. */
            dot_shape_ratio: {
                type: jspsych.ParameterType.FLOAT,
                pretty_name: "Dot shape ratio",
                default: 1.0,
            },
    ...
    class RdkPlugin {
        ...
        trial(display_element, trial) {
            ...
            var dot_shape_ratio = assignParameterValue(trial.dot_shape_ratio, 1.0);
            ...
            var dotShapeRatio = dot_shape_ratio;
            /*
            ...
            const setUpMultipleApertures = () => {
                ...
                dotShapeRatioArray = setParameter(dotShapeRatio);
            ...
            const end_trial = () => {
                ...
                var trial_data = {
                    ...
                    dot_shape: dot_shape,
                    dot_shape_ratio: dot_shape_ratio,
            ...
            function initializeCurrentApertureParameters(currentApertureNumber) {
                ...
                dotShapeRatio = dotShapeRatioArray[currentApertureNumber];
            ...
            function makeDotArray() {
                ...
                for (var i = 0; i < nDots; i++) {
                    ...
                    var dot = {
                        ...
                        // 添加颜色标记：true=红色，false=蓝色
                        isRed: (i % 2 === 0), // 偶数索引为红色，奇数索引为蓝色
                        // 添加形状标记：根据dotShapeRatio分配形状
                        shape: (Math.random() < dotShapeRatio) ? dotShape : (dotShape === "circle" ? "square" : "circle")
                    };
                    //randomly set the x and y coordinates
                    dot = resetLocation(dot);
                    // 计算各种点的数量
                    var nCoherentDots = Math.floor(nDots * coherence);
                    var nOppositeCoherentDots = Math.floor(nDots * oppositeCoherence);
            ...
            function draw() {
                ...
                for (let i = 0; i < nDots; i++) {
                    ...
                    if (colorChangeDelay === 0) {
                        if (dotColorFinal.length === 1) {
                            ctx.fillStyle = dotColorFinal[0];
                        }
                        else if (nColorsProportion === 0.5) { // 0.5的情况为运动判断，不需要根据比例分配颜色，但需要保证一致运动和随机运动的点的颜色比例一致
                             // 根据点的isRed属性分配颜色
                            if (dot.isRed) {
                                ctx.fillStyle = dotColorFinal[0];
                            } else {
                            ...
                            if (i < nColorsProportion) {
                                ctx.fillStyle = dotColorFinal[0]; // 第一种颜色 修正错误，现在没有延迟变化也能改变颜色
                            } else {
                                ctx.fillStyle = dotColorFinal[1]; // 第二种颜色
                            }
                        }
                    }
                    else {
                        if (frameCounter < colorChangeDelay) {
                ...
                //         }
                //     }
                    // 根据每个点的shape属性选择绘制函数和大小
                    let drawFn;
                    let dot_size;
                    if (dot.shape == "square") {
                        drawFn = squareFn;
                        // 混合形状时，方形的对角线 = 圆形的直径，使两者视觉面积匹配
                        // 纯方形时使用用户设定的 dot_side_length
                        dot_size = (dotShapeRatio < 1.0) ? dotRadius * Math.sqrt(Math.PI) / 2 : dotSideLength * 0.5;
                    }
                    else {
                        drawFn = circleFn;
                        dot_size = dotRadius;
